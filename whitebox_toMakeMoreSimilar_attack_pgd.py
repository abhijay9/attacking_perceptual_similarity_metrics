...
def pgd_attack(x_0, x_1, x_ref, model, model_frwd_func=True, eps=0.1, alpha=0.001, max_iters=50):
    if model_frwd_func:
    ...
    for i in range(max_iters):
        ...
        loss = loss_func(
            (s_other / (s_other + s_adv)).float(), torch.tensor([0]).float().cuda()
        )
...
# Create output file
output_file = "results/whitebox_attack_toMakeMoreSimilar/" + conf.save + "_pgd"
