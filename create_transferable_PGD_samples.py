...
def pgd_attack(x_0, x_1, x_ref, model, model_frwd_func=True, eps=0.03, alpha=0.001, max_iters=20):
    if model_frwd_func:
...
for impath in tqdm(list(stadv_samples["path"])):
    ...
    x_adv, d0, d1, dAdv, rank = pgd_attack(x_0, x_1, x_ref, metric, model_frwd_func=True, max_iters=max_pgd_attack_iters)
    
    if not os.path.isdir(results_path+"/PGD"+str(max_pgd_attack_iters)+"/"):
        os.makedirs(results_path+"/PGD"+str(max_pgd_attack_iters)+"/")

    sample_id = impath.split("/")[-3]+"_"+impath.split("/")[-1].split(".npy")[0]
    util.save_image(util.tensor2im(x_adv), results_path+"/PGD"+str(max_pgd_attack_iters)+"/"+sample_id+".png")